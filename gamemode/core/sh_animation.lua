...
function impulse.InstallAnimationMethods(meta)
    ...
    function meta:CreateAnimation(length, data)
        ...
        local animation = impulse.tween.new(
        ...
        if (length == 0 or (!animation.bIgnoreConfig and !impulse.Settings:Get("perf_anim"))) then
...
if (CLIENT) then
    ...
    impulse.InstallAnimationMethods(panelMeta)
end