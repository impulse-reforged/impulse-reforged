...
function PANEL:SetupVendor()
    ...
    if self.Vendor.Talk and impulse.Settings:Get("misc_vendorgreeting", true) then
        surface.PlaySound(impulse.Util:GetRandomAmbientVO(self.Vendor.Gender))
    end
    ...
    if vNameLbl:GetWide() >= 330 then
        vNameLbl:SetFont("Impulse-Elements20-Shadow")
        vNameLbl:SizeToContents()
    end
